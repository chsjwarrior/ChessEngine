...
static bool canMakeKingCastle(const BitBoard& bitBoard, const Color color) {
	if (bitBoard.hasCastlePermission(KING_CASTLE, color)) {
		...
		if (!hasIntersection((FILES[FILE_F] | FILES[FILE_G]) & RANKS[relativeRank], friendPieces | enemyPieces)) //isCastlePathClear
			if (!isSquareAttacked(bitBoard, ~color, getSquareOf(FILE_F, relativeRank)))//isCastlePathSecury
				return !isSquareAttacked(bitBoard, ~color, getSquareOf(FILE_G, relativeRank));
	}
	return false;
}
...
static bool canMakeQueenCastle(const BitBoard& bitBoard, const Color color) {
	if (bitBoard.hasCastlePermission(QUEEN_CASTLE, color)) {
		...
		if (!hasIntersection((FILES[FILE_B] | FILES[FILE_C] | FILES[FILE_D]) & RANKS[relativeRank], friendPieces | enemyPieces))//isCastlePathClear
			if (!isSquareAttacked(bitBoard, ~color, getSquareOf(FILE_C, relativeRank)))//isCastlePathSecury
				return !isSquareAttacked(bitBoard, ~color, getSquareOf(FILE_D, relativeRank));//isCastlePathSecury
	}
	return false;
}
...
static Bitmap getPiecesMoves(const BitBoard& bitBoard, const Piece piece, const Color color, const Square square) {
	...
	if (piece == KING)
		...
		if (piece == PAWN)
			attacks = getPawnMoves(allPieces, color, getBitmapOf(bitBoard.getEnPassantSquare()), squareBitmap);
		else {
			...
			if (piece == BISHOP)
				...
				attacks = getBishopMoves(allPieces, file, rank, squareBitmap) | getRookMoves(allPieces, file, rank, squareBitmap);
		}
	}
...
static void catalogMoves(const BitBoard& bitBoard, Move moves[], const Piece piece, const Color color, const Square from, Bitmap attacks) {
	...
	if (piece == PAWN) {
		...
		if (piece == KING)
			if (!isSquareAttacked(bitBoard, ~color, getFirstSquareOf(bitBoard.getBitmapPiece(KING, color)))) {//King is not in check
